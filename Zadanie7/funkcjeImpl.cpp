...
std::tuple<std::vector<std::vector<int>>, int,int,int>simulated_annealing(std::vector<int> S,int m, int T, int it,Time czas_start,bool K) {
    ...
    for (int j = 0; j < m / 3; j++) {
        ...
        if( j+1!= m / 3) {
            ...
            ans.push_back(v);
        }else if( j+1== m / 3) { //ostatnia liczba dodawana recznie
            ans.push_back(v);
    ...
    float a=0.95;///> stała do obliczania wychładzania
    float tempO=1000.0; ///> początkowa temperatura
    for(int k=1;k<=it;k++){
        x+=1;
        act=find_best_neighbours(ans,T,true); //losowanie sasiada
        temp=tempO*(float)pow(a,k);       //sposób chlodzenia temperatury
        ...
        if(result<=Q){
            ...
            u=static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //rand(0,1)
            if(u<exp(-1*(abs(result-Q))/temp)){
                ans=act;
                Q=result;
            }
        }
        if(krzywa){                         //wyświetlanie informacji dla kazdej iteracji
        auto stop = std::chrono::system_clock::now();
        std::chrono::duration<double> czas = stop-czas_start;
        outfile<<x<<" "<<czas.count()<<" "<<lc<<" "<<Q<<" "<<temp<<endl;
        }
    }
    return {ans, Q,x,lc};
}
