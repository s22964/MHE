...
std::pair<std::vector<std::vector<int>>, int> tabu_search(int m, int T, int it) {
    ...
    for (int k = 0; k < it; k++) {
        x+=1;
        vector<vector<int>> best_neighbour= find_best_neighbours(tabu_list.back(),T); //znalezienie sasiada i wybranie najlepszego z nich
        ...
        if(Quality(tabu_list.back(),T)<= Quality(ans,T)){
            ans=tabu_list.back();
        }
